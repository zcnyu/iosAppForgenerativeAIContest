                    ...
                    if viewModel.detailData.isEmpty {
                        ...
                        ForEach(viewModel.detailData, id: \.date) { data in
                            VStack(alignment: .leading) {
                                ...
                                let medicineTaken = data.status.medicine_taken?.map { $0 ? "✓" : "✗" }.joined(separator: ", ") ?? ""
                                let medicineEffect = data.status.medicine_effect?.map { String($0 ?? false) }.joined(separator: ", ") ?? ""
                                let medicineName = data.status.medicine_name ?? ""

                                Text("頭痛の強さ: \(headacheIntensity)")
                                Text("薬の服用: \(medicineTaken)")
                                Text("薬効: \(medicineEffect)")
                                Text("薬の名前: \(medicineName)")
